...
import { ArbitrageOpportunity, DexQuote, TokenSymbol, TOKENS, UsdPrices } from '../types';
...
const TRI_TOKENS: TokenSymbol[] = ['SOL', 'JUP', 'PENGU', 'BONK', 'USDC'];

/**
 * Generate all unique 3-token circular routes: A→B→C→A
 */
function generateTriRoutes(): [TokenSymbol, TokenSymbol, TokenSymbol][] {
  const routes: [TokenSymbol, TokenSymbol, TokenSymbol][] = [];
 ...
 */
export function scanTriangularOpportunities(
  allQuotes: Map<string, DexQuote[]>,
  usdPrices: UsdPrices,
  cfg: ScannerConfig
  ...
  for (const [a, b, c] of routes) {
    ...
    if (profitPct > MAX_REALISTIC_PROFIT_PCT) {
      logger.debug(`⚠️ Skipping suspicious triangular ${a}→${b}→${c}→${a} at ${profitPct.toFixed(2)}%`);
      continue;
    }
    ...
    if (profitPct < cfg.minProfitPct && profitUsd < cfg.minProfitUsd) continue;
    ...
    logger.info(`🔺 Triangular: ${path} | ${leg1.dex}→${leg2.dex}→${leg3.dex} | ${profitPct.toFixed(3)}% / $${profitUsd.toFixed(4)}`);
  }
